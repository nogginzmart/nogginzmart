...
function createMembersList(el) {
  ...
  members.forEach((member) => {
    ...
    output += `<tr>
        <td>${member.title}</td>
        <td><a href="${member.url}">${member.buttonUrl ? `<img src="${member.buttonUrl}" width="120"/>` : ""}<div>${prettyUrl}</div></a></td>
		${hasDescription ? `<td>${member.description ? member.description : ""}</td>` : ""}
        <td>${member.favoriteCharacter}</td>
      </tr>`;
  });
